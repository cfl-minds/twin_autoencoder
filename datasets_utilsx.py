...
def load_img_dataset(my_dir, downscaling, color):
    ...
    print(data_files)
    np.savetxt('data_files.txt', data_files, delimiter=',', header='names')
...
def predict_images(model, imgs, sols):
    ...
    #max_error = np.zeros((n_imgs,2),dtype=np.float16)
    #fail      = np.zeros((n_imgs,2),dtype=np.float16)
    mse       = np.zeros((n_imgs,2),dtype=np.float32)
    ...
    for i in range(0, n_imgs):
        ...
        #predict[i,:,:,0]   = np.clip(predict[i,:,:,0], 1e-7, 1.0-1e-7)
        #p                  = sols[i,:,:,0]*predict[i,:,:,0] + (1-sols[i,:,:,0])*(1-predict[i,:,:,0]) 
        #bcn                = -np.log(p) # * (1-error[i,:,:,0])**2
        #bcn                = (1.0-p)**4 * bcn
        #error[i,:,:,0]     = binary_focal_loss_fixed(sols[i,:,:,0], predict[i,:,:,0])
        #- sols[i,:,:,0] * np.log(predict[i,:,:,0]+10**(-5)) - (1 - sols[i,:,:,0]) *  np.log(1 - predict[i,:,:,0]+10**(-5))
        error[i,:,:,:]      = np.abs(predict[i,:,:,:]-sols[i,:,:,:])
        # error[i,:,:,1:]     = np.abs(predict[i,:,:,1:]-sols[i,:,:,1:])
        # MeanSquaredError(sols[i,:,:,1:], predict[i,:,:,1:])
        #max_error[i, :]    = [np.amax(error[i,:,:,0]), np.amax(error[i,:,:,1:])]
        #fail[i, :]         = [fail_count(predict[i,:,:,:1], sols[i,:,:,:1], 0.05), fail_count(predict[i,:,:,1:], sols[i,:,:,1:], 0.05)]
        mse[i, :]          = [np.mean(np.square(error[i,:,:,0])), np.mean(np.square(error[i,:,:,1:]))]
        #mse[i, :]            = [np.mean(bcn), np.mean(np.square(sols[i,:,:,1:] - predict[i,:,:,1:]))]
    return predict, error, mse
